...
(function () {
    ...
    tips.defaultOptions = {
        ...
        direction: 'inner',         //计算Tips定位时，inner - 计算Tips 宽高，尽量使Tips 位于父元素内部，normal - 不计算Tips宽高，按照position数据来，outer - 计算Tips 宽高，尽量使Tips 位于父元素外部, outside - 计算Tips 宽高，尽量使Tips 位于父元素外部； 默认'inner'。
    ...
    tips.prototype.setClass = function() {
        ...
        if(this.isHtmlMessage()) {
            ...
            if(C.isObj(html) && html.isParentTransparent) tipBoxClass = tipBoxClass + ' ohoTip-box-html-transparent';
        }
    ...
    /**
     * 获取Tips 的css 定位类，只有以body 为父元素的Tips 需要
     */
    ...
    tips.prototype.getTranslate = function(myTranslate, calculateTip) {
        ...
        if(this.floatOptions.hasOwnProperty('offsetType') && this.floatOptions.offsetType === '%') {
            tipTranslate.top = (eleDetails.pHeight - tipGroupHeight) * parseFloat(myTranslate.top)/100;
            tipTranslate.left = (eleDetails.pWidth - tipGroupWidth) * parseFloat(myTranslate.left)/100;
        }else {
    ...
    window.ohoTips = function(options, extendOptions = {}) {
        return new tips(options, extendOptions);
    }
