...
(function () {
     ...
     * 
     * @param    Object                 cusFloatOptions 函数内自定义浮动参数，优先级低于用户自定义浮动参数
     */
    ...
    tips.prototype.floatTranslateTypeDefault = function() {
        ...
        //优先级低于用户自定义浮动参数 this.options.float.cusFloatOptions
        let cusFloatOptions = {
            status: true,
            xOffsetLen: 0.0005,
            yOffsetLen: 0.0005,
        ...
        //优先级低于 this.options.float.cusFloatStatus
        //首次设置浮动时，this.options.float.cusFloatStatus 不存在，以后每次浮动都会记录当前浮动状态。 
        //当浏览器窗口大小发生改变时，会重新设置浮动。 此时this.options.float.cusFloatStatus已存在，则以其为准，忽略 cusFloatStatus
        let cusFloatStatus = {
            top: 0,
            left: 0,
        };
    ...
    tips.prototype.animationTypeGf = function(status) {
        ...
        let $bg = this.backup._bgElem;
        if(status == 'in') {
            C.addClass($tip, 'ohoTip-amt-GF');
            if($bg) {
                C.addClass($bg, 'ohoTip-bg-amt-GF');
            }
    ...
    tips.prototype.createGroupCopy = function() {
        ...
        let pElemStyle = C.getStyle(this.backup._pElem);
        if(pElemStyle.position == "fixed") {
            this.backup._pElem.appendChild($tipGroupClone);
        }else {
            document.body.appendChild($tipGroupClone);
        }
    ...
    tips.prototype.insertCanvas = function() {
        ...
        let tgcWidth = C.outerWidth($tipGroupClone), tgcHeight = C.outerHeight($tipGroupClone);
        ...
        if(tgcStyle.position == "fixed") {
            if(tgcStyle.right == '0px') {
                // 如果有垂直滚动条
                if(C.hasScrollbarY()) {
                    // 设置html2canvasConfig['scrollX'] 和 修改 html2canvasConfig.x 都可以达到相同的效果
                    // html2canvasConfig['scrollX'] = 0 - scrollWidth;
                    html2canvasConfig.x += scrollWidth;
                }
            }else if(tgcStyle.left != '0px') {
                // 如果有垂直滚动条
                if(C.hasScrollbarY()) {
                    // 默认居中的话， translateX 和 translateY 都是50%，经测试，translate 会计算 scroll，故此简单计算
                    html2canvasConfig.x += (scrollWidth/2);
                }
            }
        }
    ...
    tips.prototype.removeImgFragment = function($tipGroupClone) {
        let fragments = $tipGroupClone.getElementsByTagName("div");
        let xCount = Math.round(C.outerWidth($tipGroupClone)/C.outerWidth(fragments[0]));
        let yCount = Math.round(C.outerHeight($tipGroupClone)/C.outerHeight(fragments[0]));
